            ...
            <div className="max-w-[1600px] w-full p-6 md:p-12 z-10 space-y-16">
                ...
                <header className="flex justify-between items-center py-4 sticky top-0 z-50 bg-black/50 backdrop-blur-xl border-b border-white/5 -mx-6 px-6 md:-mx-12 md:px-12">
                    ...
                    <div className="flex items-center gap-6">
                        ...
                        <button
                            onClick={async () => {
                                if (!confirm("Reset stuck projects? This will mark all 'Processing' projects as 'Failed'.")) return;
                                try {
                                    await fetch("/api/proxy/admin/reset-stuck", { method: "POST" });
                                    alert("Reset Complete! Refreshing...");
                                    window.location.reload();
                                } catch (e) { alert("Reset Failed: " + e); }
                            }}
                            className="px-3 py-1.5 rounded-full bg-yellow-500/10 border border-yellow-500/20 text-yellow-400 text-xs font-medium hover:bg-yellow-500/20 transition-colors"
                        >
                            Reset Stuck
                        </button>
